...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "trial_keys":
        ...
        active_keys = sum(1 for k in trial_keys.values() if not k.get('used', False) and time.time() < float(k.get('expiry', 0)))
        used_keys = sum(1 for k in trial_keys.values() if k.get('used', False))
    ...
    if data == "approve_pending":
        ...
        if pending_users:
            for u in pending_users[:5]:
                ...
                text += f"🆔 {to_small_caps('ID')}: {u['user_id']}\n"
                text += f"📅 {u.get('request_date', 'Unknown')}\n\n"
