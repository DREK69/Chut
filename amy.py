...
def get_menu_keyboard():
    keyboard = [
        ...
        [KeyboardButton("👥 User Management"), KeyboardButton("⚙️ Bot Settings")],
        [KeyboardButton("👑 Owner Panel"), KeyboardButton("🎫 Token Management")],
        [KeyboardButton("❓ Help")]
    ]
...
Use buttons to continue..."""
    ...
    if update.message:
        ...
        await update.callback_query.message.reply_text(welcome_message, reply_markup=menu_keyboard)
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global current_attack, cooldown_until, MAINTENANCE_MODE
    user_id = update.effective_user.id
...
📈 Status: RUNNING"""
        ...
        if not is_owner(user_id):
            await update.message.reply_text("⚠️ ACCESS DENIED\nOnly owners can access bot settings.")
            return
        keyboard = get_bot_settings_keyboard()
        ...
        if not is_owner(user_id):
            await update.message.reply_text("⚠️ ACCESS DENIED\nOnly owners can access owner panel.")
            return
        keyboard = get_owner_panel_keyboard()
        ...
        if not is_owner(user_id):
            ...
            return
        keyboard = get_token_management_keyboard()
        await update.message.reply_text("🎫 TOKEN MANAGEMENT\nSelect an option:", reply_markup=keyboard)
        return
