...
async def safe_edit(q,text,reply_markup=None,is_callback=True):
 try:
  if is_callback:await q.edit_message_text(text,reply_markup=reply_markup)
  else:await q.edit_text(text,reply_markup=reply_markup)
 except Exception as e:
  if"message is not modified"not in str(e).lower():
   logger.error(f"Error: {e}")
   try:
    if is_callback:await q.message.reply_text(text,reply_markup=reply_markup)
    else:await q.reply_text(text,reply_markup=reply_markup)
   except:pass
...
async def status_menu(q):
 uid=q.from_user.id;attack_active="🔴 YES"if current_attack else"🟢 NO";cd_active="🔴 YES"if time.time()<cooldown_until else"🟢 NO"
 if current_attack:
  tleft=int(current_attack['end_time']-time.time())
