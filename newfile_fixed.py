...
👤 **Started By**: {current_attack.get('user', 'N/A')}
...
"""
    ...
    status_text += f"\n⚙️ **Server Status**:\n"
    status_text += f"🔑 Servers: {len(github_tokens)}\n"
    status_text += f"⏳ Cooldown: {COOLDOWN_DURATION}s\n"
    status_text += f"🎯 Max Attacks: {MAX_ATTACKS}\n"
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == WAITING_FOR_IP:
        ...
        try:
            ...
            # Launch attack
            await launch_attack(target, port, duration, user_id, username, context)
            user_states[user_id] = None
