...
Select an option:"""
        ...
        if not is_owner(user_id) and not is_admin(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED\nOnly admins can add users.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        await query.message.edit_text("➕ ADD USER\n\nEnter user ID and days:\nFormat: <user_id> <days>\n\nExample: 123456789 7\nUse 0 for lifetime access\n\nSend /cancel to abort.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="user_management")]]))
        context.user_data['waiting_for_add_user'] = True
    elif data == "remove_user_prompt":
        if not is_owner(user_id) and not is_admin(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED\nOnly admins can remove users.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        await query.message.edit_text("➖ REMOVE USER\n\nEnter user ID to remove:\n\nExample: 123456789\n\nSend /cancel to abort.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="user_management")]]))
        context.user_data['waiting_for_remove_user'] = True
    elif data == "users_list":
        if not is_owner(user_id) and not is_admin(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        if not pending_users:
            await query.message.edit_text("📭 No pending requests", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        users_list = "⏳ PENDING REQUESTS\n\n"
        for user in pending_users:
            users_list += f"• {user['user_id']} - @{user['username']}\n"
        users_list += f"\n📊 Total: {len(pending_users)}\nTo approve: /add <id> <days>"
        await query.message.edit_text(users_list, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
    elif data == "approved_users_list":
        if not is_owner(user_id) and not is_admin(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        if not approved_users:
            await query.message.edit_text("📭 No approved users", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        ...
        for owner_id, owner_info in owners.items():
            ...
            if is_primary:
                owners_list += " 👑 (PRIMARY)"
            owners_list += f"\n  Added by: {added_by}\n"
        await query.message.edit_text(owners_list, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
    elif data == "admin_list":
        if not is_owner(user_id) and not is_admin(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        if not admins:
            await query.message.edit_text("📭 No admins", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        admins_list = "🛡️ ADMINS LIST\n\n"
        for admin_id, admin_info in admins.items():
            username = admin_info.get('username', f'admin_{admin_id}')
            admins_list += f"• {admin_id} - @{username}\n"
        await query.message.edit_text(admins_list, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
    elif data == "reseller_list":
        if not is_owner(user_id) and not is_admin(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        if not resellers:
            await query.message.edit_text("📭 No resellers", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        ...
        await query.message.edit_text(resellers_list, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
...
Select an option:"""
        ...
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED\nOnly owners can toggle maintenance.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
            return
        global MAINTENANCE_MODE
        MAINTENANCE_MODE = not MAINTENANCE_MODE
        save_maintenance_mode(MAINTENANCE_MODE)
        status = "ENABLED" if MAINTENANCE_MODE else "DISABLED"
        await query.message.edit_text(f"🔧 MAINTENANCE MODE {status}\nMaintenance mode is now {'on' if MAINTENANCE_MODE else 'off'}.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
    elif data == "set_cooldown_prompt":
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED\nOnly owners can set cooldown.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
            return
        await query.message.edit_text("⏱️ SET COOLDOWN\n\nEnter new cooldown duration in seconds:\n\nExample: 60\nMinimum: 10 seconds\n\nSend /cancel to abort.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="bot_settings")]]))
        context.user_data['waiting_for_cooldown'] = True
    elif data == "set_max_attack_prompt":
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED\nOnly owners can set max attacks.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
            return
        ...
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED\nOnly owners can generate trial keys.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
            return
        ...
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED\nOnly owners can remove expired tokens.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
            return
        ...
        if not expired_tokens:
            await progress_msg.edit_text("✅ All tokens are valid.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
            return
        github_tokens.clear()
        ...
        await progress_msg.edit_text(expired_list, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
...
Contact admin for purchase."""
        await query.message.edit_text(price_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
...
Select an option:"""
        ...
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
            return
        await query.message.edit_text("➕ ADD OWNER\n\nEnter user ID to make owner:\n\nExample: 123456789\n\nSend /cancel to abort.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="owner_panel")]]))
        ...
        if not is_owner(user_id):
            ...
            return
        if not groups:
            await query.message.edit_text("📭 No groups registered", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
            return
        groups_list = "📊 REGISTERED GROUPS\n\n"
        for group_id, group_info in groups.items():
            groups_list += f"• {group_info.get('name', 'Unknown')}\n  ID: {group_id}\n"
        groups_list += f"\n📊 Total: {len(groups)}"
        await query.message.edit_text(groups_list, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
...
Select an option:"""
        ...
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
            return
        await query.message.edit_text("➕ ADD TOKEN\n\nEnter GitHub token:\n\nExample: ghp_xxxxxxxxxxxx\n\nSend /cancel to abort.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="token_management")]]))
        ...
        if not github_tokens:
            await query.message.edit_text("📭 No tokens added yet.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
            return
        ...
        await query.message.edit_text(tokens_list, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
    elif data == "remove_token_prompt":
        if not is_owner(user_id):
            await query.message.edit_text("⚠️ ACCESS DENIED", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
            return
        if not github_tokens:
            await query.message.edit_text("📭 No tokens to remove.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
            return
        ...
        if not is_owner(user_id):
            ...
            return
        if not github_tokens:
            await query.message.edit_text("❌ NO SERVERS AVAILABLE\nNo servers added. Use Add Token first.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
            return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('waiting_for_attack_target'):
        ...
        parts = text.split()
        if len(parts) != 2:
            await update.message.reply_text("❌ INVALID FORMAT\nFormat: <user_id> <days>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
            return
        try:
            ...
            try:
                ...
                pass
            await update.message.reply_text(f"✅ USER ADDED\n\nUser ID: {new_user_id}\nDuration: {days} days\nAdded by: {user_id}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid user ID or days", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
        ...
        try:
            ...
            if user_to_remove_str in user_attack_counts:
                ...
                save_user_attack_counts(user_attack_counts)
            if removed:
                await update.message.reply_text(f"✅ USER REMOVED\n\nUser ID: {user_to_remove}\nRemoved by: {user_id}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
                try:
                    await context.bot.send_message(chat_id=user_to_remove, text="🚫 ACCESS REMOVED\nYour access has been revoked. Contact admin.")
                except:
                    pass
            else:
                await update.message.reply_text(f"❌ USER NOT FOUND\nUser ID {user_to_remove} not found.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid user ID", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="user_management")]]))
    elif context.user_data.get('waiting_for_cooldown'):
        context.user_data['waiting_for_cooldown'] = False
        try:
            new_cooldown = int(text)
            if new_cooldown < 10:
                await update.message.reply_text("❌ Cooldown must be at least 10 seconds", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
                return
            global COOLDOWN_DURATION
            COOLDOWN_DURATION = new_cooldown
            save_cooldown(new_cooldown)
            await update.message.reply_text(f"✅ COOLDOWN UPDATED\n\nNew cooldown: {COOLDOWN_DURATION} seconds", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid number", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
    elif context.user_data.get('waiting_for_max_attack'):
        context.user_data['waiting_for_max_attack'] = False
        try:
            new_max = int(text)
            if new_max < 1:
                await update.message.reply_text("❌ Max attacks must be at least 1", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
                return
            global MAX_ATTACKS
            MAX_ATTACKS = new_max
            save_max_attacks(new_max)
            await update.message.reply_text(f"✅ MAX ATTACKS UPDATED\n\nNew maximum: {MAX_ATTACKS} attacks", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid number", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
        ...
        try:
            hours = int(text)
            if hours < 1:
                await update.message.reply_text("❌ Hours must be at least 1", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
                return
            key = generate_trial_key(hours)
            await update.message.reply_text(f"🎫 TRIAL KEY GENERATED\n\nKey: `{key}`\nDuration: {hours} hours\n\nShare this key with users.\nThey can redeem with: /redeem {key}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid number", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="bot_settings")]]))
        ...
        try:
            ...
            if str(new_owner_id) in owners:
                await update.message.reply_text("❌ Already an owner", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
                return
            ...
            save_owners(owners)
            await update.message.reply_text(f"✅ OWNER ADDED\n\nUser ID: {new_owner_id}\nAdded by: {user_id}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
            try:
                await context.bot.send_message(chat_id=new_owner_id, text="👑 OWNER ACCESS GRANTED\nYou have been given owner privileges.")
            except:
                pass
        except ValueError:
            await update.message.reply_text("❌ Invalid user ID", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
        ...
        try:
            ...
            if str(owner_to_delete) not in owners:
                await update.message.reply_text("❌ Not an owner", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
                return
            if owners[str(owner_to_delete)].get('is_primary'):
                await update.message.reply_text("❌ Cannot remove primary owner", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
                return
            del owners[str(owner_to_delete)]
            save_owners(owners)
            await update.message.reply_text(f"✅ OWNER REMOVED\n\nUser ID: {owner_to_delete}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid user ID", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
        ...
        try:
            ...
            if str(new_reseller_id) in resellers:
                await update.message.reply_text("❌ Already a reseller", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
                return
        ...
        try:
            ...
            if str(reseller_to_remove) not in resellers:
                await update.message.reply_text("❌ Not a reseller", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
                return
            del resellers[str(reseller_to_remove)]
            save_resellers(resellers)
            await update.message.reply_text(f"✅ RESELLER REMOVED\n\nUser ID: {reseller_to_remove}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid user ID", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
        ...
        for uid in list(approved_users.keys()) + list(owners.keys()) + list(admins.keys()) + list(resellers.keys()):
            try:
                ...
                failed += 1
        await progress_msg.edit_text(f"✅ BROADCAST COMPLETE\n\nSuccess: {success}\nFailed: {failed}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="owner_panel")]]))
        ...
        try:
            for existing_token in github_tokens:
                if existing_token['token'] == token:
                    await update.message.reply_text("❌ Token already exists.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
                    return
            g = Github(token)
            user = g.get_user()
            username = user.login
            ...
            await update.message.reply_text(message, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
        except Exception as e:
            await update.message.reply_text(f"❌ ERROR\n{str(e)}\nCheck token.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
        ...
        try:
            ...
            if token_num < 1 or token_num > len(github_tokens):
                await update.message.reply_text(f"❌ Invalid number. Use 1-{len(github_tokens)}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
                return
            removed_token = github_tokens.pop(token_num - 1)
            save_github_tokens(github_tokens)
            await update.message.reply_text(f"✅ SERVER REMOVED!\n\nServer: {removed_token['username']}\nRepo: {removed_token['repo']}\nRemaining: {len(github_tokens)}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
        except ValueError:
            await update.message.reply_text("❌ Invalid number", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
    elif context.user_data.get('waiting_for_binary'):
        await update.message.reply_text("❌ Please send a file, not text.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
...
async def handle_binary_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]])
        await progress_msg.edit_text(message, reply_markup=keyboard)
    except Exception as e:
        await progress_msg.edit_text(f"❌ ERROR\n{str(e)}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="token_management")]]))
